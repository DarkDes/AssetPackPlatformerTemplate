...
@onready var fps_spin = $AnimData/Grid/FPSSpin
...
func _on_sprites_lits_item_selected(index):
	...
	if "animations" in sprite_selected:
...
func _on_animations_list_item_selected(index):
	...
	# Запонить данные
	if "sprites" in asset_data.settings_data:
		var _sprites = asset_data.settings_data["sprites"]
...
func _on_fps_spin_value_changed(value):
	...
	_config[animation_selected_name]["FPS"] = value
	
	# Apply FPS to Anim
	
func get_sprite_data_for_changes(sprite_name):
	if ("sprites" in asset_data.settings_data) == false:
		asset_data.settings_data["sprites"] = {}
	var _sprites = asset_data.settings_data["sprites"]
